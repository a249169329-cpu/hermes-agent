...
DEFAULT_MAX_MANUAL_COMPRESSION_RETRIES = 3
...
def format_manual_compression_retry_prompt(
    ...
    *,
    ...
    err = str(error or "unknown error").strip() or "unknown error"
    if len(err) > 500:
        err = err[:497].rstrip() + "..."
    attempts = max(0, int(attempts or 0))
    max_attempts = max(1, int(max_attempts or DEFAULT_MAX_MANUAL_COMPRESSION_RETRIES))
    ...
    lines = [
        ...
        "",
        "回复：",
        f"- 重试 = 再试一次摘要压缩（{attempts}/{max_attempts}）",
